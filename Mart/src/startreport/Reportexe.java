...
public class Reportexe implements ITestListener {
 ...
 TakesScreenshot ts1 = (TakesScreenshot) driver;
  ...
  try {
   FileUtils.copyFile(src, new File("C:\\images1\\" + result.getMethod().getMethodName() + ".png"));
   String file = test.addScreenCapture("C:\\images1\\" + result.getMethod().getMethodName() + ".png");
   test.log(LogStatus.PASS, result.getMethod().getMethodName() + "test is passed", file);
   test.log(LogStatus.PASS, result.getMethod().getMethodName() + "test is passed", result.getThrowable().getMessage());
  } catch (IOException e) {
   e.printStackTrace();
  }
 }
